...
class AppShareClient(StreamingClient):
    """
    Class for the screen share streaming client.
    ...
    def __init__(self, host, port, appname, x_res=1024, y_res=576):
        """
        Creates a new instance of ScreenShareClient.
        ...
        y_res : int
            the y resolution
        """
        self.appid = self._find_all_windows(appname)[0]
        self.__x_res = x_res
        self.__y_res = y_res
        super(AppShareClient, self).__init__(host, port)

    def _find_all_windows(self, name):
        result = []
    ...
    def _get_frame(self):
        ...
        # if not win32gui.GetForegroundWindow() == self.appid:
        #     return 0
        bbox = win32gui.GetWindowRect(hwnd)
        appscreen = ImageGrab.grab(bbox)
        
        # screen = pyautogui.screenshot()
        frame = np.array(appscreen)
        frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        frame = cv2.resize(frame, (self.__x_res, self.__y_res), interpolation=cv2.INTER_AREA)
        return frame